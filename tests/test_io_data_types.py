...
def test_read_data_single():
    ...
    fld4.add_field(comm, field_name="w", file_type="fld", file_name=fname, file_key="vel_2", dtype=ddtype)
...
def test_wrapper():
    ...
    data = read_data(comm, fname, ["vel_0", "vel_1", "vel_2", "temp", "pres"], dtype = ddtype)
    # change the name of 2 fields
    data["temp_0"] = data.pop("temp")
    data["pres_0"] = data.pop("pres")
    ...
    for key in fld1.fields.keys():
        ...
        for j in range(len(fld1.fields[key])):
            ...
            t2 = np.allclose(fld1.fields[key][j], fld3.fields[key][j])
            t3 = np.allclose(fld2.fields[key][j], fld3.fields[key][j])
            t4 = np.allclose(fld1.fields[key][j], data[f"{key}_{j}"])
            t5 = np.allclose(fld2.fields[key][j], data[f"{key}_{j}"])
            t6 = np.allclose(fld3.fields[key][j], data[f"{key}_{j}"])
            passed3 = np.all([t1, t2, t3, t4, t5, t6])
