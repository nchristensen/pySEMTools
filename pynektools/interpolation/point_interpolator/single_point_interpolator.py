...
class SinglePointInterpolator(ABC):
        ...
        for pts in range(0, probes.shape[0]):
            if err_code[pts] != 1:
                for e in element_candidates[pts]:
                    ...
                    r, s, t = self.find_rst_from_xyz(probes[pts,0], probes[pts,1], probes[pts,2]) 
                    if self.point_inside_element:
                        ...
                        if use_test_pattern:
                            test_field = x[e,:,:,:]**2 + y[e,:,:,:]**2 + z[e,:,:,:]**2
                            test_probe = probes[pts, 0]**2 +  probes[pts, 1]**2 + probes[pts, 2]**2 
                            test_interp = self.interpolate_field_at_rst(r, s, t, test_field)
    ...
    def interpolate_field_from_rst(self, probes_info, interpolation_buffer = None, sampled_field = None, settings = None):
        ...
        for e in el_owner:
            if err_code[i] != 0 :
                #self.project_element_into_basis(x[e,:,:,:], y[e,:,:,:], z[e,:,:,:])
                tmp = self.interpolate_field_at_rst(probes_rst[i,0], probes_rst[i,1], probes_rst[i,2], sampled_field[e,:,:,:])
