...
def physical_space(pod: dict[int, POD], ioh: dict[int, IoHelp], wavenumbers: list[int], modes: list[int], field_shape: tuple, fft_axis: int, field_names: list[str], N_samples: int, snapshots: list[int] = None):
    ...
    if isinstance(snapshots, list):
        ...
        for snap_id, snapshot in enumerate(snapshots):
            ...
            fourier_field_3d = [np.zeros(field_shape, dtype=pod[0].u_1t.dtype) for i in range(0, len(field_names))]
            ...
            for kappa in wavenumbers:
                ...
                field_list1d = ioh[kappa].split_narray_to_1dfields(fourier_reconstruction[kappa][:,snap_id])
...
def pod_fourier_1_homogenous_direction(comm, file_sequence: list[str], pod_fields: list[str], mass_matrix_fname: str, mass_matrix_key: str, k: int, p: int, fft_axis: int) -> tuple:
    """
