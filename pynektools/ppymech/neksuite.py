...
def preadnek(filename, comm, data_dtype="float64"):
    ...
    data = HexaData(header.nb_dims, ioh.nelv, header.orders, header.nb_vars, 0, dtype=data_dtype)
    ...
    if ioh.pos_variables > 0:
        byte_offset = mpi_offset + ioh.offset_el * ioh.gdim * ioh.lxyz * ioh.FLD_DATA_SIZE
...
def pwritenek(filename, data, comm):
    ...
    dh = Header(data.wdsz, data.lr1, data.nel, data.nel, data.time,
               data.istep, fid=0, nb_files=1, nb_vars=data.var,
              )
    ...
    h = Header(data.wdsz, data.lr1, ioh.glb_nelv, ioh.glb_nelv, data.time,
               data.istep, fid=0, nb_files=1, nb_vars=data.var,
              )
