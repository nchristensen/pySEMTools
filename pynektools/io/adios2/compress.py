...
class DataCompressor:
    ...
    def __init__(self, comm, mesh_info=None, wrd_size=4):
        ...
        # Declare useful data
        self.glb_nelv = np.ones((1), dtype=np.intc)
        self.lx = 0
        self.ly = 0
        self.lz = 0
        self.lxyz = np.ones((1), dtype=np.intc)
        self.gdim = np.ones((1), dtype=np.intc)
        self.wrd_size = np.ones((1), dtype=np.intc) * wrd_size
        if wrd_size == 4:
