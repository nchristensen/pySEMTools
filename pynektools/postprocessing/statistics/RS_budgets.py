...
def give_me_the_stat_file_name(which_dir, fname_base, stat_file_number, \
                               which_code="NEKO", nek5000_stat_type="s"):
    if which_code.casefold()=="neko":
        output_file_name = which_dir+"/"+fname_base
    elif which_code.casefold()=="nek5000":
        output_file_name = which_dir+"/"+nek5000_stat_type+stat_file_number+fname_base
...
###########################################################################################
###########################################################################################
def compute_and_write_additional_pstat_fields(which_dir,fname_mesh,fname_mean,fname_stat,\
                                              if_write_mesh=False,which_code="NEKO",nek5000_stat_type="s", \
    ...
    if which_code.casefold()=="nek5000":
        if fname_mean != fname_stat:
            sys.exit('for NEK5000 statistics fname_mean must be the same as fname_stat and equal to casename0.fXXXXX')
        if nek5000_stat_type!="s" and nek5000_stat_type!="t":
            sys.exit('for NEK5000 statistics nek5000_stat_type can be either "s" or "t"')
    ...
    if this_ext != this_ext_check:
        warnings.warn('File index of fname_stat and fname_mean differ! Hope you know what you are doing!')
    ...
    stat_file_number_UiUjUk = [ "06" ,  "07" ,  "07" ,  "07" , "07" , "08"  ,  "09" ,  "08" , "08"  , "08" ] 
    ...
    if which_code.casefold()=="neko":
        ...
        file_keys_UiUjUk        = ["scal_2","scal_3","scal_4","scal_5","scal_6","scal_7","scal_8","scal_9","scal_10","scal_11"]
        ...
        #                           "UU"  , "VV"  , "WW"  , "UV"  , "UW"  , "VW"  
        file_keys_UiUj          = ["vel_0","vel_1","vel_2","vel_0","vel_2","vel_1"] # not a mistake! nek5000's order is a bit strange

        #                          "UUU" , "VVV" , "WWW" , "UUV" , "UUW", "UVV" , "UVW" , "VVW" , "UWW" ,"VWW"
        file_keys_UiUjUk        = ["temp","vel_0","vel_1","vel_2","temp","vel_0","vel_2","vel_1","vel_2","temp"]
    ...
    this_file_name = give_me_the_stat_file_name(which_dir, fname_mean, "01", which_code, nek5000_stat_type)
    mean_fields.add_field(comm, field_name="U", file_type="fld", file_name=this_file_name, \
                          file_key=file_keys_mean_fields[0], dtype=np.single)
    mean_fields.add_field(comm, field_name="V", file_type="fld", file_name=this_file_name, \
                          file_key=file_keys_mean_fields[1], dtype=np.single)
    mean_fields.add_field(comm, field_name="W", file_type="fld", file_name=this_file_name, \
                          file_key=file_keys_mean_fields[2], dtype=np.single)
    mean_fields.add_field(comm, field_name="P", file_type="fld", file_name=this_file_name, \
                          file_key=file_keys_mean_fields[3], dtype=np.single)
    ...
    if if_do_dssum_on_derivatives:    # this could be a terrible idea to do dssum on a vector that is then differentiated again
        if comm.Get_rank() == 0:
            warnings.warn("you are doing dssum on a derivative that you are differentiating again. maybe change this.")
        do_dssum_on_3comp_vector( dU_dxi , coef , msh )
        do_dssum_on_3comp_vector( dV_dxi , coef , msh )
        do_dssum_on_3comp_vector( dW_dxi , coef , msh )
    write_file_9c(comm, msh, dU_dxi, dV_dxi, dW_dxi, fname_gradU, if_write_mesh=if_write_mesh )
    ...
    if if_do_dssum_on_derivatives:
        ...
        do_dssum_on_3comp_vector( dV_dxi , coef , msh )
        do_dssum_on_3comp_vector( dW_dxi , coef , msh )
    write_file_9c(comm, msh, dU_dxi, dV_dxi, dW_dxi, fname_hessU, if_write_mesh=if_write_mesh )
    ...
    write_file_6c(comm, msh, dP_dxi , d2P_dxi2, fname_derivP, if_write_mesh=if_write_mesh )
    ...
    ###########################################################################################
    this_file_name = give_me_the_stat_file_name(which_dir, fname_stat, stat_file_number_PU[0], which_code, nek5000_stat_type)
    stat_fields.add_field(comm, field_name="PU", file_type="fld", file_name=this_file_name, \
                          file_key=file_keys_PU[0], dtype=np.single)
    this_file_name = give_me_the_stat_file_name(which_dir, fname_stat, stat_file_number_PU[1], which_code, nek5000_stat_type)
    stat_fields.add_field(comm, field_name="PV", file_type="fld", file_name=this_file_name, \
                          file_key=file_keys_PU[1], dtype=np.single)
    this_file_name = give_me_the_stat_file_name(which_dir, fname_stat, stat_file_number_PU[2], which_code, nek5000_stat_type)
    stat_fields.add_field(comm, field_name="PW", file_type="fld", file_name=this_file_name, \
                          file_key=file_keys_PU[2], dtype=np.single)

    compute_scalar_first_derivative(comm, msh, coef, stat_fields.registry["PU"], dPU_dxi )
    compute_scalar_first_derivative(comm, msh, coef, stat_fields.registry["PV"], dPV_dxi )
    compute_scalar_first_derivative(comm, msh, coef, stat_fields.registry["PW"], dPW_dxi )
    if if_do_dssum_on_derivatives:
    ...
    write_file_9c(comm, msh, dPU_dxi, dPV_dxi, dPW_dxi, fname_gradPU, if_write_mesh=if_write_mesh )
    ...
    for icomp in range(0, 6):
        ...
        # elif which_code.casefold()=="nek5000":
        #     this_file_name=
        this_file_name = give_me_the_stat_file_name(which_dir, fname_stat, stat_file_number_UiUj[icomp], which_code, nek5000_stat_type)
        stat_fields.add_field(comm, field_name=actual_field_names[icomp], \
                            file_type="fld", file_name=this_file_name, \
                            file_key=file_keys_UiUj[icomp], dtype=np.single)
        
        compute_scalar_first_derivative(comm, msh, coef, stat_fields.registry[actual_field_names[icomp]], dQ_dxi )
        ...
        this_file_name=which_dir+"/dn"+actual_field_names[icomp]+"dxn"+this_ext

        write_file_6c(comm, msh, dQ_dxi , d2Q_dxi2, this_file_name, if_write_mesh=if_write_mesh )
    ...
    ###########################################################################################
    actual_field_names=[ "UUU","VVV","WWW","UUV","UUW","UVV","UVW","VVW","UWW","VWW"]

    for icomp in range(0, 10):
        if comm.Get_rank() == 0:
            print("working on: "+actual_field_names[icomp])

        this_file_name = give_me_the_stat_file_name(which_dir, fname_stat, stat_file_number_UiUjUk[icomp], which_code, nek5000_stat_type)

        stat_fields.add_field(comm, field_name=actual_field_names[icomp], \
                            file_type="fld", file_name=this_file_name, \
                            file_key=file_keys_UiUjUk[icomp], dtype=np.single)
...
def user_defined_interpolating_points():
    ...
    x_bbox = [.5, 2]
    y_bbox = [-1, 1]
    z_bbox = [0 , .6] # See how here I am just setting this to be one value
...
def interpolate_all_stat_and_pstat_fields_onto_points( \
        which_dir,fname_mesh,fname_mean,fname_stat,xyz,\
        which_code="NEKO",nek5000_stat_type="s",if_do_dssum_before_interp=True, \
    ...
    if which_code.casefold()=="neko":
        these_names = [ give_me_the_stat_file_name(which_dir, fname_mean, "00", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_stat, "00", which_code, nek5000_stat_type) ]
    elif which_code.casefold()=="nek5000":
        these_names = [ give_me_the_stat_file_name(which_dir, fname_mean, "01", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "02", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "03", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "04", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "05", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "06", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "07", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "08", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "09", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "10", which_code, nek5000_stat_type) , \
                        give_me_the_stat_file_name(which_dir, fname_mean, "11", which_code, nek5000_stat_type) ]
    ...
    if if_create_boundingBox_for_interp:
        ...
        msh = mp.create_partitioned_mesh(msh, partitioning_algorithm="load_balanced_linear", create_conectivity=True)
...
#########################
        ...
        for icomp in range(0, len(field_names)):
            if comm.Get_rank() == 0:
                print("---working on field ", icomp, "from a total of ", len(field_names))

            # load the field
            mean_fields.add_field(comm, field_name="tmpF", \
                                  file_type="fld", file_name=fname, \
                                  file_key=field_names[icomp], dtype=np.single)
            
            if if_create_boundingBox_for_interp:
                mean_fields = mp.create_partitioned_field(mean_fields, partitioning_algorithm="load_balanced_linear")
            ...
            probes.interpolate_from_field_list(0, [mean_fields.registry["tmpF"]] , \
                                               comm, write_data = True) 
...
interpolate_all_stat_and_pstat_fields_onto_points( \
        which_dir,fname_mesh,fname_mean,fname_stat,xyz,\
        which_code=which_code,nek5000_stat_type=nek5000_stat_type, \
        if_do_dssum_before_interp=if_do_dssum_before_interp , if_create_boundingBox_for_interp=if_create_boundingBox_for_interp )
